...
import numpy as np
import pandas
from WebSpiderRIT.vectors import getVectorsFromKeys
from WebSpiderRIT.vectors import getVectorsFromFile
from WebSpiderRIT.Calculo import calc_sim
...
def getDocs(query):
    data = openFile("WebSpiderRIT/calc_pesos_verbose.json")
    documents = []
...
def getDocsWithAllData(docs_Ids):
    data = openFile("WebSpiderRIT/documentsFullData.json")
    documents = {}
    docs_finded = []
    for doc in data:
...
def query(queryStr):
    vectorsCollection = getVectorsFromFile("WebSpiderRIT/calc_pesos_verbose.json")
